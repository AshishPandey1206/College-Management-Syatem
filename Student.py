...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS REGISTRATIONSTUDENT (RID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, FNAME TEXT, LNAME TEXT, GENDER TEXT, ADDRESS TEXT, CONTACT TEXT,COURSE TEXT)")
...
def DisplayForm():
    ...
    display_screen.title("COLLEGE MANAGEMENT SYSTEM")
    global tree
    global SEARCH
    global fname,lname,gender,address,contact,course
    SEARCH = StringVar()
    fname = StringVar()
    lname = StringVar()
    gender = StringVar()
    ...
    #label for heading
    lbl_text = Label(TopViewForm, text="College Management System", font=('verdana', 18), width=600,bg="cyan")
    ...
    Label(LFrom, text="Course", font=("Arial", 12),bg="#15244C",fg="white").pack(side=TOP)
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "Name", "Contact", "Email","Rollno","Branch","Course"),
...
def Update():
    ...
    if fname1=='' or lname1==''or gender1=='' or address1==''or contact1==''or course1=="":
        ...
        #update query
        conn.execute('UPDATE REGISTRATIONSTUDENT SET FNAME=?,LNAME=?,GENDER=?,ADDRESS=?,CONTACT=?,COURSE=? WHERE RID = ?',(fname1,lname1,gender1,address1,contact1,course1, selecteditem[0]))
        conn.commit()
...
def register():
    ...
    if fname1=='' or lname1==''or gender1=='' or address1==''or contact1==''or course1=='':
        ...
        #execute query
        conn.execute('INSERT INTO REGISTRATIONSTUDENT (FNAME,LNAME,GENDER,ADDRESS,CONTACT,COURSE) \
              VALUES (?,?,?,?,?,?)',(fname1,lname1,gender1,address1,contact1,course1));
        conn.commit()
...
def Delete():
    ...
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            selecteditem = contents['values']
            tree.delete(curItem)
            cursor=conn.execute("DELETE FROM REGISTRATIONSTUDENT WHERE RID = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
...
def SearchRecord():
    ...
    if SEARCH.get() != "":
        ...
        cursor=conn.execute("SELECT * FROM REGISTRATIONSTUDENT WHERE FNAME LIKE ?", ('%' + str(SEARCH.get()) + '%',))
...
def DisplayData():
    ...
    tree.delete(*tree.get_children())
    #select query
    cursor=conn.execute("SELECT * FROM REGISTRATIONSTUDENT")
